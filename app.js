...
const EXERCISE_I18N = {
  ...
  function openChat(){
    ...
    openModal('🤖 '+t('coach'), `<div class="chat-config"><input id="ollama-url" value="${escapeHtml(localStorage.getItem(STORAGE.ollamaUrl())||'http://localhost:11434')}" placeholder="${t('url')}"><input id="ollama-model" value="${escapeHtml(localStorage.getItem(STORAGE.ollamaModel())||'glm-5.2:cloud')}" placeholder="${t('model')}"></div><div class="chat-messages" id="chat-messages">${chat.map(m=>`<div class="chat-msg ${m.role==='user'?'user':'bot'}">${escapeHtml(m.content)}</div>`).join('')}</div><div class="chat-input"><textarea id="chat-input" placeholder="${t('coachPlaceholder')}"></textarea><button class="primary-btn" onclick="FitnessFeatures.sendChat()">${t('send')}</button></div><p style="color:var(--muted);font-size:.72rem;margin-top:.5rem">Ollama muss erreichbar sein. Standard: localhost:11434 · Modell: glm-5.2:cloud.</p>`);
  ...
  async function sendChat(){
    ...
    const url=(document.getElementById('ollama-url').value||'http://localhost:11434').replace(/\/$/,'');const model=document.getElementById('ollama-model').value||'glm-5.2:cloud';localStorage.setItem(STORAGE.ollamaUrl(),url);localStorage.setItem(STORAGE.ollamaModel(),model);
    const history=read(STORAGE.chat(),[]);history.push({role:'user',content:msg});write(STORAGE.chat(),history);openChat();
    const box=document.getElementById('chat-messages');box.insertAdjacentHTML('beforeend','<div class="chat-msg bot">⏳ GLM-5.2 denkt …</div>');box.scrollTop=box.scrollHeight;
    try{
